...
// This is now an OFFICIAL rule (Session 7, Rule 2) after last week's live bug fix.
// Tested against: "size seven", "seven", "size 7".
const NUMBER_WORDS = {
...
function parseQuery(text){
  ...
  if(sizeMatch){
  ...
  } else if(sizeWordMatch && NUMBER_WORDS[sizeWordMatch[1]]!==undefined){
    result.size = NUMBER_WORDS[sizeWordMatch[1]];
  } else {
    ...
    if(numMatch && !lower.includes("£") ){
      ...
      if(wordSize!==null) result.size = wordSize;
    }
  }
